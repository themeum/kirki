...
if ( ! class_exists( 'XTKirki_Enqueue' ) ) {

	/**
	 * Enqueues JS & CSS assets
	 */
	class XTKirki_Enqueue {
		...
		public function customize_controls_l10n() {
			...
			wp_register_script( 'xtkirki-l10n', trailingslashit( XTKirki::$url ) . 'assets/js/l10n.js' );
			...
			foreach ( $configs as $id => $args ) {
				$l10n[ $id ] = XTKirki_l10n::get_strings( $id );
			}

			wp_localize_script( 'xtkirki-l10n', 'xtkirkiL10n', $l10n );
			wp_enqueue_script( 'xtkirki-l10n' );
		...
		public function customize_controls_enqueue_scripts() {
			...
			/* TODO: if ( $has_tooltips ) { */
				wp_enqueue_script( 'xtkirki-tooltip', trailingslashit( XTKirki::$url ) . 'assets/js/tooltip.js', array( 'jquery', 'customize-controls', 'jquery-ui-tooltip' ) );
			/* TODO: } */

			// Enqueue the reset script.
			wp_enqueue_script( 'xtkirki-reset', trailingslashit( XTKirki::$url ) . 'assets/js/reset.js', array( 'jquery', 'xtkirki-set-setting-value' ) );

			// Register xtkirki-functions.
			wp_register_script( 'xtkirki-set-setting-value', trailingslashit( XTKirki::$url ) . 'assets/js/functions/set-setting-value.js' );
			wp_register_script( 'xtkirki-validate-css-value', trailingslashit( XTKirki::$url ) . 'assets/js/functions/validate-css-value.js' );
			wp_register_script( 'xtkirki-notifications', trailingslashit( XTKirki::$url ) . 'assets/js/functions/notifications.js', array( 'xtkirki-l10n', 'xtkirki-validate-css-value' ) );

			// Register serialize.js.
			wp_register_script( 'serialize-js', trailingslashit( XTKirki::$url ) . 'assets/js/vendor/serialize.js' );

			// Register the color-alpha picker.
			wp_enqueue_style( 'wp-color-picker' );
			wp_register_script( 'wp-color-picker-alpha', trailingslashit( XTKirki::$url ) . 'assets/js/vendor/wp-color-picker-alpha.js', array( 'wp-color-picker' ), '1.2', true );

			// Register the jquery-ui-spinner.
			wp_register_script( 'jquery-ui-spinner', trailingslashit( XTKirki::$url ) . 'assets/js/vendor/jquery-ui-spinner', array( 'jquery', 'jquery-ui-core', 'jquery-ui-button' ) );

			// Register codemirror.
			wp_register_script( 'codemirror', trailingslashit( XTKirki::$url ) . 'assets/js/vendor/codemirror/lib/codemirror.js', array( 'jquery' ) );

			// Register selectize.
			wp_register_script( 'selectize', trailingslashit( XTKirki::$url ) . 'assets/js/vendor/selectize.js', array( 'jquery' ) );
			...
			$scripts = array(
				...
				'dimension'       => array( 'jquery', 'customize-base', 'xtkirki-notifications' ),
				'dropdown-pages'  => array( 'jquery', 'customize-base', 'selectize' ),
				'editor'          => array( 'jquery', 'customize-base', 'xtkirki-l10n' ),
				...
				'preset'          => array( 'jquery', 'customize-base', 'selectize', 'xtkirki-set-setting-value' ),
				'radio-buttonset' => array( 'jquery', 'customize-base' ),
				'radio-image'     => array( 'jquery', 'customize-base' ),
				'radio'           => array( 'jquery', 'customize-base' ),
				'repeater'        => array( 'jquery', 'customize-base', 'jquery-ui-core', 'jquery-ui-sortable', 'xtkirki-l10n' ),
				...
				'spacing'         => array( 'jquery', 'customize-base', 'xtkirki-notifications' ),
			...
			foreach ( $scripts as $id => $dependencies ) {
				wp_register_script( 'xtkirki-' . $id, trailingslashit( XTKirki::$url ) . 'assets/js/controls/' . $id . '.js', $dependencies, false, true );
			}

			// Add fonts to our JS objects.
			$google_fonts   = XTKirki_Fonts::get_google_fonts();
			$standard_fonts = XTKirki_Fonts::get_standard_fonts();
			$all_variants   = XTKirki_Fonts::get_all_variants();
			$all_subsets    = XTKirki_Fonts::get_google_font_subsets();
			...
			wp_localize_script( 'xtkirki-typography', 'xtkirkiAllFonts', $final );
		}
		...
		public function branding() {
			...
			if ( ! empty( $vars['logoImage'] ) || ! empty( $vars['description'] ) ) {
				wp_register_script( 'xtkirki-branding', XTKirki::$url . '/assets/js/branding.js' );
				wp_localize_script( 'xtkirki-branding', 'xtkirkiBranding', $vars );
				wp_enqueue_script( 'xtkirki-branding' );
			}
		}
		...
		public function postmessage() {
			wp_enqueue_script( 'xtkirki_auto_postmessage', trailingslashit( XTKirki::$url ) . 'assets/js/postmessage.js', array( 'customize-preview' ), false, true );
			...
			wp_localize_script( 'xtkirki_auto_postmessage', 'jsvars', $js_vars_fields );
		}
	}
}
