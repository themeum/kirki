...
if ( ! class_exists( 'XTKirki_Values' ) ) {

	/**
	 * Wrapper class for static methods.
	 */
	class XTKirki_Values {
		 ...
		 * @static
		 * @access public
		 * @param string $config_id The configuration ID. @see XTKirki_Config.
		...
		public static function get_value( $config_id = '', $field_id = '' ) {
			...
			if ( ! isset( XTKirki::$config[ $config_id ] ) ) {
				$config_id = 'global';
			}

			if ( 'theme_mod' === XTKirki::$config[ $config_id ]['option_type'] ) {

				// We're using theme_mods so just get the value using get_theme_mod.
				$default_value = null;
				if ( isset( XTKirki::$fields[ $field_id ] ) && isset( XTKirki::$fields[ $field_id ]['default'] ) ) {
					$default_value = XTKirki::$fields[ $field_id ]['default'];
				}
				$value = get_theme_mod( $field_id, $default_value );

				// If the field is a background field, then get the sub-fields
				// and return an array of the values.
				if ( isset( XTKirki::$fields[ $field_id ] ) && isset( XTKirki::$fields[ $field_id ]['type'] ) && 'background' === XTKirki::$fields[ $field_id ]['type'] ) {
					$value = array();
			...
			} elseif ( 'option' === XTKirki::$config[ $config_id ]['option_type'] ) {

				// We're using options.
				if ( '' !== XTKirki::$config[ $config_id ]['option_name'] ) {

					// Options are serialized as a single option in the db.
					// We'll have to get the option and then get the item from the array.
					$options = get_option( XTKirki::$config[ $config_id ]['option_name'] );

					if ( ! isset( XTKirki::$fields[ $field_id ] ) && isset( XTKirki::$fields[ XTKirki::$config[ $config_id ]['option_name'] . '[' . $field_id . ']' ] ) ) {
						$field_id = XTKirki::$config[ $config_id ]['option_name'] . '[' . $field_id . ']';
					}
					$setting_modified = str_replace( ']', '', str_replace( XTKirki::$config[ $config_id ]['option_name'] . '[', '', $field_id ) );

					// If this is a background field, get the individual sub-fields and return an array.
					if ( 'background' === XTKirki::$fields[ $field_id ]['type'] ) {
						$value = array();

						foreach ( XTKirki::$fields[ $field_id ]['default'] as $property => $property_default ) {
						...
						$value = ( isset( $options[ $setting_modified ] ) ) ? $options[ $setting_modified ] : XTKirki::$fields[ $field_id ]['default'];
						$value = maybe_unserialize( $value );
					}
				} else {

					// Each option separately saved in the db.
					$value = get_option( $field_id, XTKirki::$fields[ $field_id ]['default'] );

					// If the field is a background field, then get the sub-fields.
					// and return an array of the values.
					if ( 'background' === XTKirki::$fields[ $field_id ]['type'] ) {
						$value = array();
						foreach ( XTKirki::$fields[ $field_id ]['default'] as $property_key => $property_default ) {
			...
			return apply_filters( 'xtkirki/values/get_value', $value, $field_id );
