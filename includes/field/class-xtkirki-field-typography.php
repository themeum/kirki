...
if ( ! class_exists( 'XTKirki_Field_Typography' ) ) {

	/**
	 * Field overrides.
	 */
	class XTKirki_Field_Typography extends XTKirki_Field {
		...
		public static function sanitize( $value ) {
			...
			if ( isset( $value['variant'] ) || isset( $value['font-weight'] ) ) {
				...
				$valid_variants = XTKirki_Fonts::get_all_variants();
			...
			if ( isset( $value['subsets'] ) ) {
				$valid_subsets = XTKirki_Fonts::get_google_font_subsets();
				$subsets_ok = array();
			...
			if ( isset( $value['font-size'] ) && ! empty( $value['font-size'] ) ) {
				$value['font-size'] = XTKirki_Sanitize_Values::css_dimension( $value['font-size'] );
			...
			if ( isset( $value['line-height'] ) && ! empty( $value['line-height'] ) ) {
				$value['line-height'] = XTKirki_Sanitize_Values::css_dimension( $value['line-height'] );
			}

			// Sanitize the letter-spacing.
			if ( isset( $value['letter-spacing'] ) && ! empty( $value['letter-spacing'] ) ) {
				$value['letter-spacing'] = XTKirki_Sanitize_Values::css_dimension( $value['letter-spacing'] );
