...
if ( ! class_exists( 'XTKirki_Scripts_Loading' ) ) {

	/**
	 * Modifies the loading overlay.
	 */
	class XTKirki_Scripts_Loading {
		...
		public function __construct() {
			...
			// Allow disabling the custom loader using the xtkirki/config filter.
		...
		public function add_loader_to_footer() {
			?>
			<div class="xtkirki-customizer-loading-wrapper">
				<span class="xtkirki-customizer-loading"></span>
			</div>
			<?php
		}
		...
		public function add_loader_styles_to_header() {
			...
			<style>
				...
				.xtkirki-customizer-loading-wrapper {
					width: 100%;
					height: 100%;
					position: fixed;
					top: 0;
					...
					background-image: url("<?php echo esc_url_raw( XTKirki::$url ); ?>/assets/images/xtkirki-logo.svg");
					background-repeat: no-repeat;
					background-position: center center;
				}
				body.wp-customizer-unloading .xtkirki-customizer-loading-wrapper {
					display: block;
					opacity: 1;
				}
				.xtkirki-customizer-loading-wrapper .xtkirki-customizer-loading {
					position: absolute;
					width: 60px;
					height: 60px;
					top: 50%;
