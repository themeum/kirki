...
if ( ! class_exists( 'XTKirki_Explode_Background_Field' ) ) {

	/**
	 * Explodes background fields and creates sub-fields for it.
	 */
	class XTKirki_Explode_Background_Field {
		...
		public static function explode( $field ) {
			$i18n    = XTKirki_l10n::get_strings( $field['xtkirki_config'] );
				...
				switch ( $key ) {
					case 'color':
						...
						if ( isset( $field['default']['opacity'] ) && false === strpos( $value, 'rgb' ) ) {
							$value = XTKirki_Color::get_rgba( $value, $field['default']['opacity'] );
						}
						$sanitize_callback = array( 'XTKirki_Sanitize_Values', 'color' );
						break;
					case 'image':
						$type = 'image';
