...
if ( ! class_exists( 'XTKirki_Fonts_Google' ) ) {

	/**
	 * Manages the way Google Fonts are enqueued.
	 */
	final class XTKirki_Fonts_Google {

		/**
		 * The XTKirki_Fonts_Google instance.
		...
		private function __construct() {
			...
			$this->google_fonts = XTKirki_Fonts::get_google_fonts();
		 ...
		 *
		 * @return object XTKirki_Fonts_Google
		 */
		public static function get_instance() {
			if ( null === self::$instance ) {
				self::$instance = new XTKirki_Fonts_Google();
			}
		...
		public function enqueue() {
			...
			$this->fonts = apply_filters( 'xtkirki/enqueue_google_fonts', $this->fonts );
			...
			if ( '' !== $this->link ) {
				wp_enqueue_style( 'xtkirki_google_fonts', $this->link, array(), null );
			}
		}
		...
		private function loop_fields() {
			foreach ( XTKirki::$fields as $field_id => $args ) {
		...
		private function generate_google_font( $args ) {
			...
			if ( isset( $args['type'] ) && 'xtkirki-typography' === $args['type'] ) {

				// Get the value.
				$value = XTKirki_Values::get_sanitized_field_value( $args );
				...
				if ( isset( $value['subsets'] ) ) {
					...
					// Add the subset directly to the array of subsets in the XTKirki_GoogleFonts_Manager object.
