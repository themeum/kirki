...
if ( ! class_exists( 'XTKirki_Styles_Output_CSS' ) ) {

	/**
	 * Handles CSS output.
	 */
	final class XTKirki_Styles_Output_CSS {
		...
		private function __construct() {
			if ( is_null( self::$google_fonts ) ) {
				self::$google_fonts = XTKirki_Fonts::get_google_fonts();
			}
			if ( is_null( self::$backup_fonts ) ) {
				self::$backup_fonts = XTKirki_Fonts::get_backup_fonts();
			}
		}
		...
		public static function get_instance() {
			if ( null === self::$instance ) {
				self::$instance = new XTKirki_Styles_Output_CSS();
			}
		...
		public static function css( $field ) {
			...
			self::$value = XTKirki_Values::get_sanitized_field_value( $field );

			// Find the class that will handle the outpout for this field.
			$classname = 'XTKirki_Output';
			$field_output_classes = apply_filters( 'xtkirki/' . $field['xtkirki_config'] . '/output/control-classnames', array(
				'xtkirki-spacing'    => 'XTKirki_Output_Field_Spacing',
				'xtkirki-typography' => 'XTKirki_Output_Field_Typography',
				'xtkirki-multicolor' => 'XTKirki_Output_Field_Multicolor',
			) );
			if ( array_key_exists( self::$field_type, $field_output_classes ) ) {
				$classname = $field_output_classes[ self::$field_type ];
			}
			$obj = new $classname( $field['xtkirki_config'], self::$output, self::$value );
		...
		public static function styles_parse( $css = array() ) {
			...
			// Pass our styles from the xtkirki/styles_array filter.
			$css = apply_filters( 'xtkirki/styles_array', $css );
