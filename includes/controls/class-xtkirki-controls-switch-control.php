...
if ( ! class_exists( 'XTKirki_Controls_Switch_Control' ) ) {

	/**
	 * Switch control (modified checkbox).
	 */
	class XTKirki_Controls_Switch_Control extends XTKirki_Controls_Checkbox_Control {
		...
		public function enqueue() {
			wp_enqueue_script( 'xtkirki-switch' );
		}
		...
		protected function content_template() {
			...
			<div class="switch<# if ( data.choices['round'] ) { #> round<# } #>">
				...
				<# } #>
				<input {{{ data.inputAttrs }}} name="switch_{{ data.id }}" id="switch_{{ data.id }}" type="checkbox" value="{{ data.value }}" {{{ data.link }}}<# if ( '1' == data.value ) { #> checked<# } #> />
