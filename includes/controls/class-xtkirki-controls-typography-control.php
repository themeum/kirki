...
if ( ! class_exists( 'XTKirki_Controls_Typography_Control' ) ) {

	/**
	 * Typography control.
	 */
	class XTKirki_Controls_Typography_Control extends XTKirki_Customize_Control {
		 ...
		 * @var string
		 */
		public $type = 'xtkirki-typography';
		...
		public function enqueue() {
			wp_enqueue_script( 'xtkirki-typography' );
		}
		...
		public function to_json() {
			...
			$this->json['show_variants'] = ( true === XTKirki_Fonts_Google::$force_load_all_variants ) ? false : true;
			$this->json['show_subsets']  = ( true === XTKirki_Fonts_Google::$force_load_all_subsets ) ? false : true;
		}
			...
			<div class="wrapper">
				...
				<# if ( data.default['font-family'] ) { #>
					...
					<div class="font-family">
						...
						<select {{{ data.inputAttrs }}} id="xtkirki-typography-font-family-{{{ data.id }}}" placeholder="{{ data.l10n['select-font-family'] }}"></select>
					</div>
					<# if ( true === data.show_variants || false !== data.default.variant ) { #>
						<div class="variant hide-on-standard-fonts xtkirki-variant-wrapper">
							<h5>{{ data.l10n['variant'] }}</h5>
							<select {{{ data.inputAttrs }}} class="variant" id="xtkirki-typography-variant-{{{ data.id }}}"></select>
						</div>
					<# } #>
					<# if ( true === data.show_subsets ) { #>
						<div class="subsets hide-on-standard-fonts xtkirki-subsets-wrapper">
							<h5>{{ data.l10n['subsets'] }}</h5>
							<select {{{ data.inputAttrs }}} class="subset" id="xtkirki-typography-subsets-{{{ data.id }}}"></select>
						</div>
					<# } #>
				<# } #>
				...
				<# if ( data.default['text-transform'] ) { #>
					<div class="text-transform">
						...
						<select {{{ data.inputAttrs }}} id="xtkirki-typography-text-transform-{{{ data.id }}}">
				...
				<# if ( data.default['color'] ) { #>
					<div class="color">
						...
						<input {{{ data.inputAttrs }}} type="text" data-palette="{{ data.palette }}" data-default-color="{{ data.default['color'] }}" value="{{ data.value['color'] }}" class="xtkirki-color-control color-picker" {{{ data.link }}} />
					</div>
				<# } #>
			</div>
			<?php
