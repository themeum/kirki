...
class Kirki_Output_Property_Background_Position extends Kirki_Output_Property {
	...
	protected function process_value() {
		...
		if ( false !== strpos( $this->value, ' ' ) ) {
			...
			! in_array( $x, $x_dimensions ) && $x = Kirki_Sanitize_Values::css_dimension( $x );
			// If y is not top/center/bottom, we need to sanitize it.
			! in_array( $y, $y_dimensions ) && $y = Kirki_Sanitize_Values::css_dimension( $y );
		...
		foreach ( $x_dimensions as $x_dimension ) {
			false !== strpos( $this->value, $x_dimension ) && $x = $x_dimension;
		}

		$y = 'center';
		foreach ( $y_dimensions as $y_dimension ) {
			false !== strpos( $this->value, $y_dimension ) && $y = $y_dimension;
		}
		$this->value = $x . ' ' . $y;
