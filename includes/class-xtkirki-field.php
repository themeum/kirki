...
if ( ! class_exists( 'XTKirki_Field' ) ) {
	...
	class XTKirki_Field {
		...
		/**
		 * The ID of the xtkirki_config we're using.
		 *
		 * @see XTKirki_Config
		 * @access protected
		 * @var string
		 */
		protected $xtkirki_config = 'global';
		 ...
		 * @var string
		 */
		protected $type = 'xtkirki-generic';
		 ...
		 *
		 * @see https://xtkirki.org/docs/arguments/output
		 * @access protected
		 * @var array
		 */
		 ...
		 *
		 * @see https://xtkirki.org/docs/arguments/js_vars
		 * @access protected
		 * @var array
		 */
		 ...
		 *
		 * @see https://xtkirki.org/docs/arguments/variables
		 * @access protected
		 * @var array
		 */
		 ...
		 * Then it adds the field to XTKirki::$fields.
		 *
		 * @access public
		 * @param string $config_id    The ID of the config we want to use.
		 *                             Defaults to "global".
		 *                             Configs are handled by the XTKirki_Config class.
		...
		public function __construct( $config_id = 'global', $args = array() ) {
			...
			if ( isset( $args['setting'] ) && ! empty( $args['setting'] ) && ( ! isset( $args['settings'] ) || empty( $args['settings'] ) ) ) {
				...
				unset( $args['setting'] );
				error_log( 'XTKirki: Typo found in field ' . $args['settings'] . ' ("setting" instead of "settings").' );
			}

			if ( is_string( $config_id ) ) {
				$args['xtkirki_config'] = $config_id;
			}
			...
			if ( is_array( $config_id ) && empty( $args ) ) {
				$args = $config_id;
				$this->xtkirki_config = 'global';
			}
			$this->xtkirki_config = trim( esc_attr( $config_id ) );
			if ( '' === $config_id ) {
				$this->xtkirki_config = 'global';
			}
			...
			$config_defaults = ( isset( XTKirki::$config['global'] ) ) ? XTKirki::$config['global'] : array();
			if ( 'global' !== $this->xtkirki_config && isset( XTKirki::$config[ $this->xtkirki_config ] ) ) {
				$config_defaults = XTKirki::$config[ $this->xtkirki_config ];
			}
			...
			$whitelisted = apply_filters( 'xtkirki/' . $this->xtkirki_config . '/fields/properties_whitelist', array(
		...
		protected function set_field( $whitelisted_properties = array() ) {
			...
			XTKirki::$fields[ $this->settings ] = $args;

			if ( 'background' === $this->type ) {
				// Build the background fields.
				XTKirki::$fields = XTKirki_Explode_Background_Field::process_fields( XTKirki::$fields );
			}
		...
		/**
		 * Escape $xtkirki_config.
		 *
		 * @access protected
		 */
		protected function set_xtkirki_config() {

			$this->xtkirki_config = esc_attr( $this->xtkirki_config );
		...
		protected function set_settings() {
			...
			if ( ! is_array( $this->settings ) ) {
				$this->settings = array( 'xtkirki_placeholder_setting' => $this->settings );
			}
			$settings = array();
			...
			if ( isset( $this->settings['xtkirki_placeholder_setting'] ) ) {
				$this->settings = $this->settings['xtkirki_placeholder_setting'];
			}
		...
		protected function set_active_callback() {
			...
			if ( ! empty( $this->required ) ) {
				$this->active_callback = array( 'XTKirki_Active_Callback', 'evaluate' );
				return;
			}
		...
		protected function set_sanitize_callback() {
			...
			$default_callbacks = array(
				'xtkirki-multicheck'       => array( 'XTKirki_Sanitize_Values', 'multicheck' ),
				'xtkirki-sortable'         => array( 'XTKirki_Sanitize_Values', 'sortable' ),
				'xtkirki-typography'       => array( 'XTKirki_Sanitize_Values', 'typography' ),
			);
		...
		protected function set_output() {
			...
			foreach ( $this->output as $output ) {
				...
				if ( ! isset( $output['property'] ) && ! in_array( $this->type, array( 'xtkirki-typography', 'background' ), true ) ) {
					continue;
				}
