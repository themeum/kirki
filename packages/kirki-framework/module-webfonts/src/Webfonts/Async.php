...
final class Async {
	...
	public function webfont_loader() {
		...
		$subsets = apply_filters( 'kirki/fonts/webfonts/async/subsets', 'cyrillic,cyrillic-ext,devanagari,greek,greek-ext,khmer,latin,latin-ext,vietnamese,hebrew,arabic,bengali,gujarati,tamil,telugu,thai' );
		...
		foreach ( $this->googlefonts->fonts as $font => $weights ) {
			...
			$this->fonts_to_load[] = $font . ':' . join( ',', $weights ) . ':' . $subsets;
		}
