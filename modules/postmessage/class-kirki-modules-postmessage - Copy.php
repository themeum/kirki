...
class Kirki_Modules_PostMessage {
	...
	protected function script_var_typography_advanced( $args, $field ) {
		...
		if ( use_media_queries )
		{
			...
			for ( var i in devices )
			{
				...
				{
					if ( _.isUndefined( device_val[prop] ) || !device_val[prop] )
						return false;
		...
		else
		{
			...
			properties.forEach( function( prop )
			{
				if ( _.isUndefined( newval['global'][prop] ) || !newval['global'][prop] )
					return false;
	 ...
	 * @param array $args  The arguments for this js_var.
	 */
	protected function script_var_image( $args, $field ) {
	...
	protected function script_var_spacing_advanced( $args, $field ) {
		$args = $this->get_args( $args );
		$element = $args['element'];
		$property = $args['property'];
		$breakpoints = Kirki::get_config_param( $field['kirki_config'], 'media_queries' );
